...
CSG2.prototype = {
  ...
  getBounds: function() {
    if(!this.bounds) {
    	    if(this.polygons) {
    	    	    this.bounds = CSG2.getBounds(this.polygons);
    	    } else {
    	    	    var big = Number.MAX_VALUE;
    	    	    this.bounds = { min: CSG2.Vertex(big,big,big), max: CSG2.Vertex(-big,-big,-big) }
    	    	    this.bsp.getBounds(this.bounds.min,this.bounds.max);
    	    }
    }
    return this.bounds;
  },
  ...
  getBSP: function() {
    if(!this.bsp)
      this.bsp = CSG2.Node.fromPolygons(this.polygons,this.getBounds());
    return this.bsp;
  },
